"""
DAG: fraud_detection_training_with_ab_testing
Description: DAG for periodic training of fraud detection model with separate A/B testing step.
"""
...
with DAG(
    dag_id="training_pipeline_with_ab_testing",
    default_args=default_args,
    description="Periodic training of fraud detection model with A/B testing",
    schedule_interval=timedelta(minutes=60),
    start_date=datetime(2025, 3, 27),
    catchup=False,
    tags=['mlops', 'ab_testing'],
) as dag:
    ...
    create_spark_cluster = DataprocCreateClusterOperator(
        ...
        cluster_description="YC Temporary cluster for model training and A/B testing",
    ...
    # Запуск PySpark задания для обучения модели (без A/B тестирования)
    train_model = DataprocCreatePysparkJobOperator(
        task_id="train_model",
        main_python_file_uri=f"{S3_SRC_BUCKET}/train.py",  # Updated train.py without A/B testing
        connection_id=YC_SA_CONNECTION.conn_id,
        dag=dag,
        args=[
    ...
    # Запуск PySpark задания для A/B тестирования
    ab_testing = DataprocCreatePysparkJobOperator(
        task_id="ab_testing",
        main_python_file_uri=f"{S3_SRC_BUCKET}/ab_test.py",  # New A/B testing script
        connection_id=YC_SA_CONNECTION.conn_id,
        dag=dag,
        args=[
            "--input", f"{S3_INPUT_DATA_BUCKET}/2019-08-22.parquet",
            ...
            "--experiment-name", MLFLOW_EXPERIMENT_NAME,
            "--traffic-split", "0.5",
            "--test-name", f"ab_test_{datetime.now().strftime('%Y%m%d_%H%M')}",
            "--auto-promote",
            "--s3-endpoint-url", S3_ENDPOINT_URL,
            "--s3-access-key", S3_ACCESS_KEY,
            "--s3-secret-key", S3_SECRET_KEY,
